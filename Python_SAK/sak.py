...
class Sak:
    ...
    ___________
        ...
        SAK(API_URL: str, API_KEY: str, AioHTTPSession)
        ...
        try:
            async with self.session.get(
                ...
                if resp.status in (401, 403):
                    raise InvalidApiKey(
                        "Invalid API key, Get an api key from @SAKRo_bot"
                    )
                elif resp.status == 429:
                    raise RateLimitExceeded(
                        "Rate limit exceeded, https://t.me/SakirBey2"
                    )
                response = await resp.json()
        except TimeoutErr:
            raise Exception("Failed to communicate with SAK server.")
    ...
    async def sakir(self, query: str, id: int = 0):
        """
        Returns An Object.
        ...
        """
        return await self._fetch("sakir", query=query, id=id)
